...
def _isiterable(var):
    ...
    """
    return (isinstance(var, list)) or (
        isinstance(var, set)) or (isinstance(var, tuple))
...
def alt(*args):
    ...
    """
    l = [functools.reduce(lambda x, y: x | y, a) for a in args if _isiterable(
        a) or isinstance(a, types.GeneratorType)]
    l += [a for a in args if not _isiterable(a)
...
class _SchedElement(object):
    ...
    def __init__(self, name=''):
        ...
        if name.translate(trans) != name:
            raise Exception(
                'ERROR: name %s contains one of the following characters: -+[] ->/' % name)
        self.name = name
...
class _SchedElementAffine(_DICT_TYPE):
    ...
    def __init__(self, unknown=None, element_class=_SchedElement,
                 affine_operator='+'):
        ...
        if unknown is None:
            ...
            raise Exception('ERROR: cant init %s from %s' %
                            (str(self), str(unknown)))
    ...
    def __add__(self, other):
        ...
        raise Exception('ERROR: you cannot add %s to %s' %
                        (str(other), str(self)))
    ...
    def __mul__(self, other):
        if isinstance(other, type(self)):
            ...
            if 1 in other:
                ...
                raise Exception('ERROR: can multipy %s only with integer but not with %s' % (
                    str(self), str(other)))
            return new
        else:
            return self * type(self)(other)
    ...
    def __str__(self):
        def format_coeff(val):
            if val != 1:
                return str(val) + 'x'
            return ''
        return self.affine_operator.join(
            [format_coeff(self[key]) + str(key) for key in self])

    def __repr__(self):
        # add id because representation is otherwise not unique
        return self.__str__() + '_' + str(id(self))
...
length    : length of task, default is 1
        ...
        if name in self._tasks or name in self._resources:
            raise Exception(
                'ERROR: resource or task with name %s already contained in scenario' % str(name))
    ...
    def tasks(self, resource=None):
        ...
        if resource is None:
            ...
            return list({T for T in self.tasks()
                         for RA in T.resources_req if resource in RA})
    ...
    def solution(self):
        ...
        solution = [(T, R, T.start_value, T.start_value + T.length)
                    for T in self.tasks() if T.start_value is not None and T.resources is not None
    ...
    def objective(self):
        ...
        """
        tasks_objective = [T * T['completion_time_cost']
                           for T in self.tasks() if 'completion_time_cost' in T]
        if tasks_objective:
    ...
    def use_makespan_objective(self):
        ...
        if 'MakeSpan' in self._tasks:
            self._constraints = [C for C in self._constraints if self._tasks[
                'MakeSpan'] not in C.tasks()]
            del self._tasks['MakeSpan']
        tasks = self.tasks()  # save tasks before adding makespan
        makespan = self.Task('MakeSpan')
        # add first resource, every task needs one
    ...
    def constraints(self, constraint_class=None):
        ...
        return [C for C in self._constraints if isinstance(
            C, constraint_class)]
    ...
    def add_constraint(self, constraint):
        for task in constraint.tasks():
            if task not in self:
                raise Exception('ERROR: task %s is not contained in scenario %s' % (
                    str(task.name), str(self.name)))
        for resource in constraint.resources():
            if resource not in self:
                raise Exception('ERROR: resource %s is not contained in scenario %s' % (
    ...
    def remove_constraint(self, constraint):
        if str(constraint) not in [str(C) for C in self._constraints]:
            raise Exception('ERROR: constraint %s not contained in scenario %s' % (
                str(constraint), str(self.name)))
        self._constraints = [
            C for C in self._constraints if str(C) != str(constraint)]

    def add_task(self, task):
        if task.name in self._tasks and task is not self._tasks[task.name]:
            raise Exception('ERROR: task with name %s already contained in scenario %s' % (
                str(task.name), str(self.name)))
    ...
    def remove_task(self, task):
        if task.name in self._tasks:
            ...
            raise Exception('ERROR: task with name %s not contained in scenario %s' % (
                str(task.name), str(self.name)))
        self._constraints = [
            C for C in self._constraints if task not in C.tasks()]
    ...
    def add_resource(self, resource):
        if resource.name in self._resources and resource is not self._resources[
                resource.name]:
    ...
    def remove_resource(self, resource):
        if resource.name in self._resources:
            ...
            raise Exception('ERROR: resource with name %s not contained in scenario %s' %
                            (str(resource.name), str(self.name)))
        self._constraints = [
            C for C in self._constraints if resource not in C.resources()]
    ...
    def __iadd__(self, other):
        if _isiterable(other):
            ...
            return self
        raise Exception('ERROR: cant add %s to scenario %s' %
                        (str(other), str(self.name)))
    ...
    def __isub__(self, other):
        if _isiterable(other):
            ...
            raise Exception('ERROR: cant subtract %s to scenario %s' %
                            (str(other), str(self.name)))
        return self
    ...
    def __contains__(self, item):
        if isinstance(item, Task):
            ...
            raise Exception('ERROR: %s cannot be checked for containment in scenario %s' % (
    ...
    def check(self):
        ...
        for T in self.tasks():
            if not T.resources_req:
                raise Exception(
                    'ERROR: task %s has no resource requirement' % str(T))
    ...
    def __str__(self):
        ...
        for T in self.tasks():
            s += '%s : %s\n' % (str(T.name),
                                ','.join([str(RA) for RA in T.resources_req]))
        s += '\n'
        ...
        for RA in ra_to_tasks:
            if len(RA) < 2:
                continue
            s += '%s : %s\n' % (str(RA),
                                ','.join([str(T) for T in ra_to_tasks[RA]]))
        s += '\n'
        ...
        s += print_constraint('CAPACITY DIFF LOWER BOUNDS',
                              self.capacity_diff_low())
        s += print_constraint('CAPACITY DIFF UPPER BOUNDS',
                              self.capacity_diff_up())
        s += '###############################################'
        return s
...
class Task(_SchedElement):
    ...
    def __init__(self, name, length=1, group=None, required=True, **kwargs):
        ...
        # does this job need to get scheduled, False makes sense if it has
        # negative cap requirement
        self.required = True
    ...
    def remove_resources_req(self, RA):
        self.resources_req = [
            RA_ for RA_ in self.resources_req if str(RA_) != str(RA)]
        return self
    ...
    def __iadd__(self, other):
        if _isiterable(other):
            ...
            self.add_resources_req(other)
            return self
        raise Exception('ERROR: cant add %s to task %s' %
                        (str(other), str(self)))
    ...
    def __isub__(self, other):
        if _isiterable(other):
            ...
            self.remove_resources_req(other)
            return self
        raise Exception('ERROR: cant subtract %s from task %s' %
                        (str(other), str(self)))
...
class _TaskAffine(_SchedElementAffine):
    ...
    def _get_prec(self, TA, comp_operator):
        ...
        if pos_tasks and neg_tasks:
            ...
            right = neg_tasks[0]
            if comp_operator == '<':
                return PrecedenceLax(
                    task_left=left, task_right=right, offset=offset)
            elif comp_operator == '<=':
                return PrecedenceTight(
                    task_left=left, task_right=right, offset=offset)
            elif comp_operator == '<<':
                return PrecedenceCond(
                    task_left=left, task_right=right, offset=offset)
...
class _Bound(_Constraint):
    ...
    def __repr__(self):
        return str(self.task) + ' ' + str(self.comp_operator) + \
            ' ' + str(self.bound)
...
class _ResourceAffine(_SchedElementAffine):
    ...
    def __init__(self, unknown=None):
        _SchedElementAffine.__init__(
            self, unknown=unknown, element_class=Resource, affine_operator='|')

    def __or__(self, other):
        # add of superclass
        return super(_ResourceAffine, self).__add__(_ResourceAffine(other))
...
class _Capacity(_Constraint):
    ...
    def __ge__(self, other):
        if not _isnumeric(other):
            raise Exception(
                'ERROR: %s is not an integer, only integers are allowed' % str(other))
    ...
    def __getitem__(self, key):
        ...
        if 'T' in weight_.__code__.co_varnames and 't' in weight_.__code__.co_varnames:
            ...
            raise Exception(
                'ERROR: wrong parameters in capacity selection function')
