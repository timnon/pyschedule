...
def plot(scenario, img_filename=None, resource_height=1.0, show_task_labels=True,
    ...
    # color map
    colors = ['#7EA7D8', '#A1D372', '#EB4845',
              '#7BCDC8', '#FFF79A']  # pastel colors
    # colors = ['red','green','blue','yellow','orange','black','purple']
    # #basic colors
    colors += [[random.random() for i in range(3)]
               for x in range(len(S.tasks()))]  # random colors
    ...
    solution = [(T, R, x, y)
                for (T, R, x, y) in solution if T not in hide_tasks_str]
    ...
    for R in visible_resources:
        ...
        for T, R_, x, y in R_solution:
            sorted_levels = sorted(
                level_fill.items(), key=operator.itemgetter(1, 0))
        ...
        for (T, R, x, x_) in R_solution:
            for level in task_levels[T]:
                y = (total_resource_sizes - 1 -
                     (resource_sizes_count + level)) * resource_height
                ax.add_patch(
                    patches.Rectangle(
                        (x, y),       # (x,y)
                        max(x_ - x, 0.5),   # width
                ...
                if show_task_labels:
                    plt.text(x, y + 0.1 * resource_height,
                             str(T.name), fontsize=14, color='black')
        resource_sizes_count += resource_size

    # format graph
    plt.title(str(S.name))
    plt.yticks([resource_height * x + resource_height /
                2.0 for x in range(len(R_ticks))], R_ticks[::-1])
    # resource_height*len(resources))
    plt.ylim(0, resource_sizes_count * resource_height)
    plt.xlim(
        0, max([x_ for (I, R, x, x_) in solution if R in visible_resources]))
    if img_filename:
        fig.figsize = (1, 1)
