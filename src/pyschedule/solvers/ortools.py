...
def solve(scenario, time_limit=None, copy_scenario=False, msg=0):
    ...
    resource_to_intervals = resource_to_intervals = {
        R: list() for R in S.resources()}
    resource_task_to_interval = collections.OrderedDict()
    for T in S.tasks():
        I = ort_solver.FixedDurationIntervalVar(
            0, S.horizon - T.length, T.length, False, T.name)
    ...
    for T in S.tasks():
        ...
        for RA in T.resources_req:
            RA_tasks = list()
            for R in RA:
                I_ = ort_solver.FixedDurationIntervalVar(
                    0, S.horizon - T.length, T.length, True, T.name + '_' + R.name)
            ...
            ort_solver.Add(ort_solver.Sum(
                [I_.PerformedExpr() for I_ in RA_tasks]) == 1)
    ...
    for R in S.resources():
        disj = ort_solver.DisjunctiveConstraint(
            resource_to_intervals[R], R.name)
        sequences[R] = disj.SequenceVar()
        ort_solver.Add(disj)

    # move objective
    # TODO: bug, variables that are not part of the objective might not be
    # finally defined
    ...
    for P in S.precs_lax():
        ort_solver.Add(task_to_interval[P.task_right].StartsAfterEnd(
            task_to_interval[P.task_left]))
        # TODO: add offset, but this requires DependecyGraph which is not
        # exposed via swig?

    # precedences tight
    for P in S.precs_tight():
        ort_solver.Add(task_to_interval[P.task_right].StartsAtEnd(
            task_to_interval[P.task_left]))
        # TODO: add offset, but this requires DependecyGraph which is not
        # exposed via swig?
    ...
    for C in S.capacity_low():
        ...
        if C._start != None or C._end != None:
            continue
        R = C.resource
        cap_tasks = [(resource_task_to_interval[R, T], C.weight(T=T, t=0))
                     for T in S.tasks()]
        ort_solver.Add(ort_solver.Sum(
            [I.PerformedExpr() * w for I, w in cap_tasks]) >= C.bound)
    ...
    limits = (ort_solver.Limit(ort_time_limit, branch_limit,
                               failures_limit, solutions_limit, True))
    ...
    # read last solution
    for T in S.tasks():
        # collector.StartValue(0, task_to_interval[T])
