...
def solve_discrete_hop(scenario, horizon, kind='CBC', time_limit=None, task_groups=None, msg=0, return_copy=False):
	"""
	Shortcut to discrete mip
	"""
	if return_copy:
		scenario = copy.deepcopy(scenario)
	DiscreteMIPHop().solve(scenario, horizon, kind=kind, time_limit=time_limit, task_groups=task_groups, msg=msg)
	return scenario
	...
	def build_mip_from_scenario(self, msg=0):
		...
		for T in self.task_groups:
			...
			if T.start is None or task_group_size > 1:
				...
				cons.append(
					pl.LpConstraint(x[T, self.horizon], sense=0, rhs=0))  # required for no solution feedback
				...
				if starts and max(starts) > self.horizon+1:
					raise Exception(
						'ERROR: fixed start of task ' + str(T) + ' larger than max time step ' + str(self.horizon+1))
				for t in starts:
					starts_after_t = [t_ for t_ in starts if t_ >= t]
					cons.append(pl.LpConstraint(pl.LpAffineExpression([(x[T, t], 1), (x[T, t + 1], -1)]), sense=1,
												rhs=len(starts_after_t)))
				...
				for R in resources:
					if R in S.resources(task=T):
						cons.append(
							pl.LpConstraint(pl.LpAffineExpression([(x[T, R, 0], 1)]), sense=1, rhs=resources.count(R)))
					else:
						raise Exception(
		...
		for R in S.resources():
			...
			for t in range(self.horizon+1):
				affine = pl.LpAffineExpression(
					[(x[T, R, max(t - T.length, 0)], S.resources_req_coeff(task=T, resource=R)) for T in
					 resource_tasks] + \
		...
		for P in S.precs_lax():
			if P.left in self.task_groups_free and P.right in self.task_groups_free:
				...
				if P.left in self.task_groups_free and P.right in self.task_groups_free:  # TODO: take care of all other cases -> treat as prec_low and prec_up
					for t in range(self.horizon+1):
						affine = pl.LpAffineExpression([( x[P.left, t], 1 / left_size), (
		...
		for P in S.precs_cond():
			if P.left in self.task_groups_free and P.right in self.task_groups_free:
				...
				for r in shared_resources:
					for t in range(self.horizon+1):
						affine = pl.LpAffineExpression(
							[( x[P.left, r, max(t - P.left.length, 0)], 1 ), (x[P.left, r, t], -1), \
...
class DiscreteMIPHop(object):
	...
	def build_mip_from_scenario(self, msg=0):
		...
		for T in self.task_groups:
			...
			# base time-indexed variables for task group
			for t in range(self.horizon):
				x[T, t] = pl.LpVariable(str((T, t)), 0, task_group_size, cat=pl.LpInteger)
				#x[T, t] = pl.LpVariable(str((T, t)), 0, 1, cat=pl.LpBinary)
			affine = pl.LpAffineExpression([(x[T, t], 1) for t in range(self.horizon) ])
			cons.append(pl.LpConstraint(affine, sense=0, rhs=1))
			# lower and upper boundary conditions

			# generate variables for task resources
			resources = set(S.resources(task=T)) - set(S.resources(task=T,single_resource=True))
			for R in resources:
				# resource base variables
				for t in range(self.horizon):
			...
			RAs = set(S.resources_req(task=T)) - set(S.resources_req(task=T,single_resource=True))
			for RA in RAs:
				# one position needs to get selected
				affine = pl.LpAffineExpression([(x[T, R, t], 1) for R in RA for t in range(self.horizon) ])
				cons.append(pl.LpConstraint(affine, sense=0, rhs=1))
			...
			# fix variables if start is given
			starts = [ (T_.start,R) for T_ in self.task_groups[T] if T_.start is not None for R in T_.resources ]
			for (t,R) in set(starts):
				mult = starts.count((t,R))
				if (T,R,t) in x:
					mip += x[T,R,t] == mult
				else:
					mip += x[T,t] == mult
		...
		# resource non-overlapping constraints
		for R in S.resources():
			resource_tasks = S.tasks(resource=R)
			resource_tasks_single = S.tasks(resource=R,single_resource=True)
			resource_tasks = [ T for T in resource_tasks if T not in resource_tasks_single ]
			if R.size is not None:
				resource_size = R.size
			else:
				resource_size = 1.0
			for t in range(self.horizon):
				affine = pl.LpAffineExpression(
					[(x[T, R, t], S.resources_req_coeff(task=T, resource=R)) for T in resource_tasks] +
				    [(x[T, t], S.resources_req_coeff(task=T, resource=R)) for T in resource_tasks_single ])
				cons.append(pl.LpConstraint(affine, sense=-1, rhs=resource_size))

		# lax precedence constraints
		for P in S.precs_lax():
			# first constraint
			affine = pl.LpAffineExpression([(x[P.left, t],t) for t in range(self.horizon)] + [(x[P.right, t],-t) for t in range(self.horizon)])
			cons.append(pl.LpConstraint(affine, sense=-1, rhs=-(P.left.length + P.offset)))
			# second constraint
			for t in range(self.horizon):
				affine = pl.LpAffineExpression([(x[P.left, t_],1) for t_ in range(t,self.horizon)] + [(x[P.right, t_],-11) for t_ in range(min(t+P.left.length+P.offset,self.horizon),self.horizon)])
				cons.append(pl.LpConstraint(affine, sense=-1, rhs=0))

		# tight precedence constraints
		for P in S.precs_tight():
			affine = pl.LpAffineExpression([(x[P.left, t],t) for t in range(self.horizon)] + [(x[P.right, t],-t) for t in range(self.horizon)])
			cons.append(pl.LpConstraint(affine, sense=0, rhs=-(P.left.length + P.offset)))

		# low precedence constraints
		for P in S.precs_low():
			affine = pl.LpAffineExpression([(x[P.left, t],t) for t in range(self.horizon)])
			cons.append(pl.LpConstraint(affine, sense=1, rhs=P.right))

		# up precedence constraints
		for P in S.precs_up():
			affine = pl.LpAffineExpression([(x[P.left, t],t) for t in range(self.horizon)])
			cons.append(pl.LpConstraint(affine, sense=-1, rhs=P.right-P.left.length))

		# capacity lower bounds
		for C in S.capacity_low():
			R = C.resource
			param = C.param
			start, end = max(0,C.start), min(self.horizon-1,C.end)
...
class DiscreteMIPSingle(object):
	"""
	pulp with time discretisation
	"""
	...
	def build_mip_from_scenario(self, msg=0):
		...
		for T in self.task_groups:
			...
			if T.start is None or task_group_size > 1:
				...
				cons.append(pl.LpConstraint(x[T, 0], sense=0, rhs=task_group_size))
				cons.append(pl.LpConstraint(x[T, self.horizon], sense=0, rhs=0))  # required for no solution feedback
		...
		# make sure that tasks with similar alternative resource are scheduled to the same resource
		RAs = set(S.resources_req()) - set(S.resources_req(single_resource=True))
		for RA in RAs:
			tasks = [T for T in RA.tasks() if T in self.task_groups_free]
			if len(tasks) > 1 :
				T = tasks[0]
		...
		for R in S.resources():
			...
			resource_tasks_single = [ T for T in S.tasks(resource=R,single_resource=True) if T in self.task_groups_free ]
			resource_tasks = [ T for T in resource_tasks if T not in resource_tasks_single ]
			if R.size is not None:
				resource_size = R.size
			else:
			...
			for t in range(self.horizon+1):
				affine = pl.LpAffineExpression(
					[(x[T, R, t-T.length],  S.resources_req_coeff(task=T, resource=R)) for T in resource_tasks if T.length <= t] +
					[(x[T, R, t], -S.resources_req_coeff(task=T, resource=R)) for T in resource_tasks if T.length <= t ] +
					[(x[T, t-T.length], S.resources_req_coeff(task=T, resource=R)) for T in resource_tasks_single if T.length <= t ] +
				    [(x[T, t], -S.resources_req_coeff(task=T, resource=R)) for T in resource_tasks_single if T.length <= t ] )
				cons.append(pl.LpConstraint(affine, sense=-1, rhs=resource_size))
		...
		for P in S.precs_cond():
			if P.left in self.task_groups_free and P.right in self.task_groups_free:
				...
				right_size = float(len(self.task_groups[P.right]))
				S.resources(task=T)
				shared_resources = set(S.resources(task=P.left)) & set(S.resources(task=P.right))
				shared_resources_single = set(S.resources(task=P.left,single_resource=True)) & set(S.resources(task=P.right,single_resource=True))
				shared_resources = shared_resources - shared_resources_single
				for R in shared_resources:
					for t in range(self.horizon):
						affine = pl.LpAffineExpression(
							[( x[P.left, R, t], 1 ), (x[P.left, R, t+1], -1),
							 ( x[P.right, R, t], 1),( x[P.right, R, min(t+P.offset,self.horizon) ], -1)])
						cons.append(pl.LpConstraint(affine, sense=-1, rhs=1))
				if shared_resources_single:
					for t in range(self.horizon):
						affine = pl.LpAffineExpression(
							[( x[P.left, t], 1 ), (x[P.left, t+1], -1),
							 ( x[P.right, t], 1),( x[P.right, min(t+P.offset,self.horizon) ], -1)])
						cons.append(pl.LpConstraint(affine, sense=-1, rhs=1))
		...
		for C in S.capacity_up():
			...
			start, end = max(0,C.start), min(self.horizon,C.end)
			tasks = [ T for T in S.tasks(resource=R) if param in T and T in self.task_groups_free ]
			tasks_single = [ T for T in S.tasks(resource=R,single_resource=True) if param in T and T in self.task_groups_free ]
			tasks = set(tasks) - set(tasks_single)
			affine = pl.LpAffineExpression([(x[T, R, start], T[param]) for T in tasks]+
										   [(x[T, R, end], -T[param]) for T in tasks]+
										   [(x[T, start], T[param]) for T in tasks_single]+
										   [(x[T, end], -T[param]) for T in tasks_single])
			cons.append(pl.LpConstraint(affine, sense=-1, rhs=C.bound))
	...
	def read_solution_from_mip(self, msg=0):
		...
		for T in self.task_groups_free:
			...
			for R in self.scenario.resources(task=T):
				if R not in single_resources:
					starts_ = [max([t for t in range(self.horizon+1) if self.x[(T, R, t)].varValue >= z - 0.5]) \
					for z in range(int(self.x[(T, R, 0)].varValue), 0, -1)]
				else:
					starts_ = [max([t for t in range(self.horizon+1) if self.x[(T, t)].varValue >= z - 0.5]) \
					for z in range(int(self.x[(T, 0)].varValue), 0, -1)]
				starts.extend([(t, R) for t in starts_])
