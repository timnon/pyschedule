...
def _solve_mip(mip,kind='CBC',time_limit=None,msg=0) :
	...
	if kind == 'CPLEX' :
		mip.solve(pulp.CPLEX_CMD(msg=msg,timelimit=time_limit))
	elif kind == 'GLPK' :
		mip.solve(pulp.GLPK_CMD(msg=msg))
	elif kind == 'CBC' :
		options = []
		if time_limit :
...
class ContinuousMIP(object) :
	...
	def build_mip_from_scenario(self,msg=0) :
		...
		mip = pulp.LpProblem(str(S), pulp.LpMinimize)
		...
		for T in S.tasks() :
			...
			for R in T.resources_req.resources() :
				x[(T,R)] = pulp.LpVariable((T,R),0,1,cat=pulp.LpBinary)
		...
		for (T,T_) in task_pairs :
			...
			if shared_resources :
				x[(T,T_,'SameResource')] = pulp.LpVariable((T,T_,'SameResource'),lowBound=0)#,cat=pulp.LpInteger)
				x[(T_,T,'SameResource')] = pulp.LpVariable((T_,T,'SameResource'),lowBound=0)#,cat=pulp.LpInteger)
				mip += x[(T,T_,'SameResource')] == x[(T_,T,'SameResource')]
				for R in shared_resources :
					mip += x[(T,R)] + x[(T_,R)] - 1 <= x[(T,T_,'SameResource')]
				# ordering variables
				x[(T,T_)] = pulp.LpVariable((T,T_),0,1,cat=pulp.LpBinary)
				x[(T_,T)] = pulp.LpVariable((T_,T),0,1,cat=pulp.LpBinary)
				mip += x[(T,T_)] + x[(T_,T)] == 1
	...
	def build_mip_from_scenario(self,msg=0) :
		...
		for T in S.tasks() :
			for t in range(MAX_TIME) :
				x[(T,t)] = pulp.LpVariable((T,t),0,1)
			# lower boundary conditions
			cons.append( pulp.LpConstraint( x[(T,0)], sense=0, rhs=1 ) )
			cons.append( pulp.LpConstraint( x[(T,self.max_time_step-1)], sense=0, rhs=0 ) ) #required for no solution feedback
	
			# fix variables if start is given
			if T.start :
				if T.start > self.max_time_step :
					raise Exception('ERROR: fixed start of task '+str(T)+' larger than max time step '+str(max_time_step))
				cons.append( pulp.LpConstraint( x[(T,T.start)], sense=0, rhs=1 ) )

			# generate variabels for task resources
			for R in T.resources_req.resources() :
				# variables
				for t in range(MAX_TIME) :
					x[(T,R,t)] = pulp.LpVariable((T,R,t),0,1,cat=pulp.LpBinary)

				# monotonicity
				for t in range(MAX_TIME-1) :
					cons.append( pulp.LpConstraint( pulp.LpAffineExpression([ (x[(T,R,t)],1), (x[(T,R,t+1)],-1) ]), sense=1, rhs=0 ) )

			# consider each resource selection
			for RA in T.resources_req :
				# sum up resource distribution for each resource selection
				for t in range(MAX_TIME) :
					affine = pulp.LpAffineExpression([ (x[(T,R,t)],1) for R in RA ] + [(x[T,t],-1)])
					cons.append( pulp.LpConstraint( affine, sense=0, rhs=0 ) )

		# objective
		mip += pulp.LpAffineExpression([ ( x[(T,t)], S.objective[T]) for T in S.objective for t in range(MAX_TIME)  ] )

		# resource non-overlapping constraints 
		for R in S.resources() :
			for t in range(MAX_TIME-1) :
				resource_tasks = [ T for T in S.tasks() if R in T.resources_req.resources() ]
				affine = pulp.LpAffineExpression([ (x[(T,R,max(t-T.length,0))], 1) for T in resource_tasks ] + \
                                                                 [ (x[(T,R,t)], -1) for T in resource_tasks ])
				con =  pulp.LpConstraint( affine, sense=-1, rhs=1.0 )
				cons.append(con)
				#prob += sum([ x[(T,max(t-T.length,0))]-x[(T,t)] for T in resource_tasks ]) <= 1.0

			if R.capacity :
				resource_tasks = [ T for T in S.tasks() if R in T.resources_req.resources() ]
				affine = pulp.LpAffineExpression([ (x[T,R,0],T.resources_req.capacity(R)) for T in resource_tasks ])
				con =  pulp.LpConstraint( affine, sense=-1, rhs=R.capacity )
				cons.append(con)
		...
		for P in S.precs_tight() :
			for t in range(MAX_TIME) :
				affine = pulp.LpAffineExpression([ ( x[(P.left,t)], 1), (x[(P.right,min(t+P.left.length,MAX_TIME-1))],-1)  ])
				con = pulp.LpConstraint( affine, sense=0, rhs=0 )
				cons.append(con)
		...
		for P in S.precs_cond() :
			...
			for R in shared_resources :	
				for t in range(MAX_TIME) :
					affine = pulp.LpAffineExpression([ ( x[(P.left,R,max(t-P.left.length,0) )],1 ), (x[(P.left,R,t)],-1),\
		                                                          ( x[(P.right,R,max(t-P.left.length,0))], 1), (x[(P.right,R,min(t+P.offset,MAX_TIME-1) )],-1) ])
					con = pulp.LpConstraint( affine, sense=-1, rhs=1 )
					cons.append(con)
