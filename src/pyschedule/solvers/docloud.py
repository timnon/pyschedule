...
class _DOcloud:
    ...
    def __init__(self, base_url, api_key, msg=0):
        ...
        self.headers_post = {'X-IBM-Client-ID': api_key,
                             'Content-Type': 'application/json'}
        self.headers_put = {'X-IBM-Client-ID': api_key,
    ...
    def execute(self, filenames):
        ...
        r = requests.post(self.url, headers=self.headers_post,
    ...
    def get_result(self, job):
        ...
        if status == 'FAILED':
            ...
            r = requests.get('%s/attachments/solution.json/blob' %
                             job, headers=self.headers_post)
            if r.status_code != 200:
