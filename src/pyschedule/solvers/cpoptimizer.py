...
def _get_dat_filename(scenario, msg=0):
    ...
    id_to_task = dict(zip(range(len(tasks)), tasks))  # unused?
    task_to_id = dict(zip(tasks, range(len(tasks))))

    LARGE_NUMBER = 1000

    # TODO: only upper capacity constraints
    Resources = [(resource_to_id[R], 0, LARGE_NUMBER)
                 for R in resources if R.size == 1]
    ...
    for T in tasks:
        ...
        single_resources = {
            R for RA in T.resources_req if len(RA) < 2 for R in RA}
            ...
            for R in RA:
                ...
                if R.size == 1:
                    ...
                    if (task_id, task_resource_group_id) not in TaskResourceGroups:
                        TaskResourceGroups.append(
                            (task_id, task_resource_group_id))
                else:
    ...
    for P in S.precs_lax():
        if P.offset >= 0:
            Precedences.append(
                (task_to_id[P.task_left], task_to_id[P.task_right], P.offset))
        elif P.offset < 0:
            Precedences.append((task_to_id[P.task_left], task_to_id[
                               P.task_right], P.offset - P.task_left.length - P.task_right.length))

    TightPrecedences = list()
    for P in S.precs_tight():
        if P.offset >= 0:
            TightPrecedences.append(
                (task_to_id[P.task_left], task_to_id[P.task_right], P.offset))
        elif P.offset < 0:
            TightPrecedences.append((task_to_id[P.task_left], task_to_id[
                                    P.task_right], P.offset - P.task_left.length - P.task_right.length))

    CondPrecedences = list()
    for P in S.precs_cond():
        CondPrecedences.append(
            (task_to_id[P.task_left], task_to_id[P.task_right], P.offset))
    ...
    CapacitySliceUpTasks = list()    # unused?
    count = 0
    for C in S.capacity_up():
        if C._start is None and C._end is None:
            CapacityUps.append(
                (count, resource_to_id[C.resource], C.bound, -1, -1))
            for T in S.tasks():
                if C.weight(T):
...
def _read_solution(scenario, log, task_to_id, id_to_resource, msg=0):
    ...
    try:
        ...
        start_i, end_i = log.index(start_str) + \
            len(start_str), log.index(end_str)
    except:
        print(log)
        if msg:
...
def solve(scenario, mod_filename=None, msg=0):
    ...
    dat_filename, task_to_id, id_to_resource = _get_dat_filename(
        scenario, msg=msg)
...
def solve_docloud(scenario, api_key,
    ...
    from . import docloud
    log = docloud.solve(base_url=base_url, api_key=api_key, filenames=[
                        mod_filename, dat_filename], msg=msg)
    if msg:
        print(log)
    return _read_solution(S, log, task_to_id, id_to_resource, msg=msg)
