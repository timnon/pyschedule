...
class ContinuousMIP(object):
    ...
    def build_mip_from_scenario(self, task_groups=None, msg=0):
        ...
        for T in S.tasks():
            # pl.LpVariable(str(T), 0)
            x[T] = mip.var(str(T), up=self.horizon, cat='Continuous')
            # add task vs resource variabls
            for RA in T.resources_req:
                for R in RA:
                    # pl.LpVariable(str((T, R)), 0, 1, cat=pl.LpBinary)
                    x[(T, R)] = mip.var(str((T, R)))
        ...
        task_pairs = [(T, T_) for T in S.tasks()
                      for T_ in S.tasks() if str(T) < str(T_)]
        for (T, T_) in task_pairs:
            ...
            if shared_resources:
                x[(T, T_, 'SameResource')] = \
                    mip.var(str((T, T_, 'SameResource')),
                            up=S.horizon, cat='Integer')
                # pl.LpVariable(str((T, T_, 'SameResource')), lowBound=0)  #
                # ,cat=pl.LpInteger)
                x[(T_, T, 'SameResource')] = \
                    mip.var(str((T_, T, 'SameResource')),
                            up=S.horizon, cat='Integer')
                # pl.LpVariable(str((T_, T, 'SameResource')), lowBound=0)  #
                # ,cat=pl.LpInteger)
                affine = [(x[(T, T_, 'SameResource')], 1),
                          (x[(T_, T, 'SameResource')], -1)]
                cons.append(mip.con(affine, sense=0, rhs=0))
                #mip += x[(T, T_, 'SameResource')] == x[(T_, T, 'SameResource')]
                for R in shared_resources:
                    affine = [(x[(T, R)], 1), (x[(T_, R)], 1),
                              (x[(T, T_, 'SameResource')], -1)]
                    cons.append(mip.con(affine, sense=-1, rhs=1))
                    #mip += x[(T, R)] + x[(T_, R)] - 1 <= x[(T, T_, 'SameResource')]
                # ordering variables
                # pl.LpVariable(str((T, T_)), 0, 1, cat=pl.LpBinary)
                x[(T, T_)] = mip.var(str((T, T_)))
                # pl.LpVariable(str((T_, T)), 0, 1, cat=pl.LpBinary)
                x[(T_, T)] = mip.var(str((T_, T)))
                affine = [(x[(T, T_)], 1), (x[(T_, T)], 1)]
                cons.append(mip.con(affine, sense=0, rhs=1))
                #mip += x[(T, T_)] + x[(T_, T)] == 1

                affine = [(x[T], 1), (x[T_], -1), (x[(T, T_)], BIGM),
                          (x[(T, T_, 'SameResource')], BIGM)]
                cons.append(mip.con(affine, sense=-1, rhs=2 * BIGM - T.length))
                # mip += x[T] + T.length <= x[T_] + \
            #           (1 - x[(T, T_)]) * BIGM + (1 - x[(T, T_, 'SameResource')]) * BIGM
                affine = [(x[T_], 1), (x[T], -1), (x[(T, T_)], -BIGM),
                          (x[(T, T_, 'SameResource')], BIGM)]
        ...
        for P in S.precs_lax():
            if P.offset >= 0:
                ...
                cons.append(mip.con(affine, sense=-1, rhs=-
                                    P.task_left.length - P.offset))
                #mip += x[P.task_left] + P.task_left.length + P.offset <= x[P.task_right]
            elif P.offset < 0:
                affine = [(x[P.task_left], 1), (x[P.task_right], -1)]
                cons.append(mip.con(affine, sense=-1,
                                    rhs=P.task_right.length - P.offset))
        ...
        for P in S.precs_tight():
            if P.offset >= 0:
                affine = [(x[P.task_left], 1), (x[P.task_right], -1)]
                cons.append(mip.con(affine, sense=0, rhs=-
                                    P.task_left.length - P.offset))
                #mip += x[P.task_left] + P.task_left.length + P.offset == x[P.task_right]
            elif P.offset < 0:
                affine = [(x[P.task_left], 1), (x[P.task_right], -1)]
                cons.append(mip.con(affine, sense=0,
                                    rhs=P.task_right.length - P.offset))
                #mip += x[P.task_left] == x[P.task_right] + P.task_right.length - P.offset
        ...
        for P in S.precs_cond():
            ...
            cons.append(mip.con(affine, sense=-1, rhs=-
                                P.task_left.length - P.offset + 2 * BIGM))
            # mip += x[P.task_left] + P.task_left.length + P.offset <= x[P.task_right] + \
#                   (1 - x[(P.task_left, P.task_right)]) * BIGM + (1 - x[
    ...
    def read_solution_from_mip(self, msg=0):
        for T in self.scenario.tasks():
            ...
            T.resources = [R for R in resources if self.mip.value(self.x[
                                                                  (T, R)]) > 0]
...
class DiscreteMIP(object):
    ...
    def build_mip_from_scenario(self, msg=0):
        ...
        for T in self.task_groups:
            ...
            if task_group_size > 1:
                cat = 'Integer'
            x.update({(T, t): mip.var(str((T, t)), 0, task_group_size, cat)
        ...
        # everybody is finished before the horizon TODO: why is this check
        # necessary
        affine = [(x[T, t], 1) for T in S.tasks() for t in range(
            self.horizon - T.length + 1, self.horizon) if (T, t) in x]
        cons.append(mip.con(affine, sense=-1, rhs=0))

        coeffs = {(T, R): RA[R] for T in S.tasks()
            ...
            if P.offset >= 0:
                for t in range(self.horizon):
                    affine = [(x[P.task_left, t_], 1) for t_ in range(t, self.horizon)] + \
                             [(x[P.task_right, t_], -1) for t_ in range(min(t +
                                                                            P.task_left.length + P.offset, self.horizon), self.horizon)]
                    cons.append(mip.con(affine, sense=-1, rhs=0))
            elif P.offset < 0:
                for t in range(self.horizon):
                    affine = [(x[P.task_right, t_], 1) for t_ in range(t)] + \
                             [(x[P.task_left, t_], -1) for t_ in range(min(t +
                                                                           P.task_right.length - P.offset, self.horizon))]
                    cons.append(mip.con(affine, sense=-1, rhs=0))
        ...
        for P in S.bounds_up():
            if P.task not in self.task_groups:
                continue
            affine = [(x[P.task, t], 1) for t in range(
                max(P.bound - P.task.length + 1, 0), self.horizon)]
            cons.append(mip.con(affine, sense=0, rhs=0))
        ...
        for P in S.bounds_low_tight():
            ...
            cons.append(mip.con([(x[P.task, P.bound], 1)],
                                sense=0, rhs=task_group_size))
        ...
        for P in S.bounds_up_tight():
            ...
            task_group_size = len(self.task_groups[P.task])
            cons.append(mip.con(
                [(x[P.task, max(P.bound - P.task.length, 0)], 1)], sense=0, rhs=task_group_size))

        # conditional precedence constraints
        for P in S.precs_cond():
            if P.task_left not in self.task_groups or P.task_right not in self.task_groups:
                continue
            shared_resources = list(set(S.resources(task=P.task_left)) & set(
                S.resources(task=P.task_right)))
            for R in shared_resources:
                for t in range(self.horizon):
                    ...
                    rhs = (
                        len(self.task_groups[P.task_left]) + len(self.task_groups[P.task_right]) / 2.0)
        ...
        for C in S.capacity_diff_up():
            ...
            periods = list(set([t for t in range(self.horizon)
                                for T in self.task_groups if C.weight(T, t)]))
            ...
            for t in periods[:-1]:
                ...
                if C.kind == 'diff' or C.kind == 'diff_inc':
                    ...
                    cons.append(mip.con(affine, sense=1, rhs=0))
            affine = [(x['switch_%i' % count, R, t], 1)
                      for t in periods[:-1] if ('switch_%i' % count, R, t) in x]
            cons.append(mip.con(affine, sense=-1, rhs=C.bound))
            count += 1
    ...
    def read_solution_from_mip(self, msg=0):
        ...
        for T in self.task_groups:
            ...
            for T_ in self.task_groups[T]:
                # in case of not required tasks, there might be less starts
                # than tasks
                if not starts:
                    break
                ...
                for RA in RAs:
                    if set(T_.resources) & set(RA):
                        continue
                    (t, R) = [(t_, R_) for (t_, R_)
                              in starts if R_ in RA and t_ == T_.start_value][0]
                    starts.remove((t, R))
    ...
    def solve(self, scenario, kind='CBC', time_limit=None, random_seed=None, ratio_gap=0.0, msg=0):
        ...
        if self.scenario.horizon is None:
            raise Exception(
                'ERROR: solver requires scenarios with defined horizon')
            return 0
        self.horizon = self.scenario.horizon
        self.build_mip_from_scenario(msg=msg)
